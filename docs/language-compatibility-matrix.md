...
| Construct | Status | Reason / fix milestone |
|---|---|---|
| `;` `\|` `,` `&` separators | implemented | **v0.13.6**: fork-thunk emitter gaps closed — thunks can now write chunk-top vars via the realm-slot path (design-risks v0.13.5-A fixed); every statement form reachable inside a `&` LHS operand now compiles and runs correctly (design-risks v0.13.5-C fixed and generalized: the `&` emitter adopts the RHS closure register as a hidden declared local for the inline LHS compile, so nested `while`/`switch`/for-each/multi-statement blocks and locals declared inside the operand no longer clobber it or get clobbered). **v0.13.5**: statement forms as `&`/`\|` operands — blocks and `if`/`while` statement forms fold as operands (`{ a } & { b }`); `switch` as a `&`/`\|` operand stays a parse error (`expected expression` — not admitted by `parse_sep_operand`); `var` declarations are rejected as operands; `,` inside tag-scope bodies unsupported; see `tests/chk/separator/block_operands.chk` + `tests/chk/separator/fork_operand_forms.chk` |
...
| `Object.asString` universal fallback | implemented | **v0.13.5** — C-native on the Object root: every UObject renders (`"<object 0x...>"`); atom types shadow with their own `asString`. **v0.13.6**: Boolean and Nil receivers now render too (overlay in `bool_nil_overlay.u`: `Boolean.asString` delegates to `toString` → `"true"`/`"false"`; `Nil.asString` returns the literal `"nil"`), so `asString` is total across the value domain; see `tests/chk/stdlib/runtime/object_asstring.chk` and `tests/chk/stdlib/runtime/bool_nil_asstring.chk`; matrix-row: stdlib-asstring |
