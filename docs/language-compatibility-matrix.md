...
| `switch ... default:` catch-all arm | implemented | **v0.13.5** — runs when no case matches; dispatch is source-position-independent (a `default` listed first still loses to a matching case); a second `default` arm is a compile error; lowered onto the existing patch-list jump machinery, no new opcode. Known issue: `switch` (and for-each) inside a `try` body miscompiles — the hidden-local floor math collides the switch subject and the case temp on one register (both land in R2 → `EQ R2,R2`, first arm always runs); pre-existing at the branch base, spec record `tests/chk/control_transfer/try_body_hidden_local_collision.chk`, tracked in workspace-root design-risks v0.13.5-E (v0.13.6 target). See `tests/chk/control_transfer/switch_default.chk`; matrix-row: syntax-switch-default |
