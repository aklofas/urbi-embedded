...
| `List` index-out-of-range (`get`/`set`/`l[i]`) | implemented | **v0.13.5 (STD-02)** — raises a catchable `IndexError` (a `LookupError` subclass) instead of a generic `TypeError`; discriminable in a `catch` guard via `isA(IndexError)`; see `tests/chk/exceptions/typed_exceptions.chk`; matrix-row: stdlib-indexerror |
| `String` char-position out-of-range (`charAt`/`asciiAt`) | implemented | **v0.13.5 (STD-02)** — raises a catchable `RangeError` instead of a generic `TypeError`; see `tests/chk/exceptions/typed_exceptions.chk`; matrix-row: stdlib-rangeerror |
| `Dict.get(missing)` / `d[missing]` | **documented divergence (STD-06)** | Returns `nil`; does NOT raise (legacy `Dictionary::get` raised `KeyError` "missing key: %s", dictionary.cc:106). v0.13.5 keeps the nil-return contract deliberately — `has(k)` gates presence, `getWithDefault` is the value-with-fallback idiom. Consequence: `KeyError` is declared in the hierarchy (a `LookupError` subclass, script-throwable) but has NO C raise site at v1.0; see `tests/chk/exceptions/typed_exceptions.chk`; matrix-row: stdlib-dict-get-nil |
