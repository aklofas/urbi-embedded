...
| `v0.2.0-expressions` | Lua-FuncState-adapted register allocator with locals/blocks/closures (cascading upvalue capture). Per-parameter `lazy` keyword (sub-proto compilation; implicit force-on-read). Statement separators: `;`/`\|` ship full runtime; `,`/`&` ship parse-only (runtime at v0.3.0). Minimum control-flow surface: `var`, `if`/`else`, `while`, `function` defs, calls, `return`, comparison ops, `true`/`false`/`nil`. Bytecode v1.0 → v1.1 (rejects v1.0 modules). Per-VM intern table + multi-VM isolation matrix. Strings, lists, dicts, object literals, methods, time/angle literals, `for`/`do-while` deferred to M2.5/v0.4.0/v0.5.0.                                | `[ ]` |
