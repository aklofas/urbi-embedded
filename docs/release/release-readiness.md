...
| ABI freeze pin | **done (W2)** | `make test-abi-freeze` — "ABI freeze pin in sync: 0/23/0" | **passing-evidence** — `_Static_assert` in `include/urbi/version.h`; v0.10.6-stabilization froze at 0/18/0; v0.10.7-audit-followup bumped PATCH to 0/18/1 (no public surface change); v0.10.8-string-concat bumped PATCH to 0/18/2 (S-string-plus runtime String + String concat — atom fast path in OP_ADD; no public surface change); v0.10.9-tag-state bumped MINOR to 0/19/0 (D1 SUSPENDED-machinery ratification — 4 new public symbols `urbi_tag_block`/`_unblock`/`_freeze`/`_unfreeze`; first post-freeze MINOR break per §3); v0.10.10-job-introspection bumped PATCH to 0/19/1 (D7 full-ship Cat. E ratification — all new surface script-side, zero new public C API symbols); v0.10.11-channel-and-isA bumped PATCH to 0/19/2 (D6+isA+D5 Cat. E ratification — Channel proto, cout/cerr/clog, <<, isA(), Object unfreeze — all script-side, zero new public C API symbols); v0.10.12-cat-e-activation bumped PATCH to 0/19/3 (final tag of 4-tag Cat. E ratification arc — fixture-and-doc-only tag, no functional changes, zero new public C API symbols); v0.10.13-hygiene bumped PATCH to 0/19/4 (post-Cat. E hygiene + targeted runtime bug fix — markdownlint config, make all + urbi CLI, String.asString stdlib overlay, slot-change first-install double-fire suppression; zero new public C API symbols); v0.10.14-prerc-infra bumped PATCH to 0/19/5 (pre-v1.0-rc stabilization arc — STYLE.md doc correction, REPL output-backpressure liveness fix, C `.chk` host-driver; zero new public C API symbols); v0.10.15-vm-decomp-2 bumped PATCH to 0/19/6 (final tag of the pre-v1.0-rc stabilization arc — VM dispatch extraction round 2 into `uvm_tag_scope` + `uvm_reactive_install`, plus v0.10.9-B user-tag scope binding and v0.10.7-B tag.stop()/finally fix; zero new public C API symbols); v0.11.0-trace-spine bumped MINOR to 0/20/0 (first tag of the v0.11.x tooling arc — new public EXPERIMENTAL header `include/urbi/trace.h` with the trace control/drain/stats API + `URBI_TP` macros, compile-gated by `URBI_TRACE`; 24th use of the pre-v1.0 escape clause; MINOR because new public surface, no new opcodes, wire unchanged); v0.11.1-perf-counters bumped PATCH to 0/20/1 (second tag of the v0.11.x tooling arc — VM-domain perf counters behind `URBI_PERF_COUNTERS`, always-on GC cycle/slice/timing, filled `Debug.profile()`/`Debug.gc()` seam + `Debug.profileReset()`; counters internal + excluded from the determinism checksum; 25th use of the pre-v1.0 escape clause; PATCH because zero new public C API symbols, no new opcodes, wire unchanged); v0.11.2-host-tooling bumped PATCH to 0/20/2 (third tag of the v0.11.x tooling arc — host-side tooling only: Python URBT→Chrome-Trace decoder, GDB pretty-printers + walkers, `--trace`/`--trace-out` capture CLI via `make urbi-trace`, `--dump-on-fatal`; the CLI flags use only already-exported public symbols; 26th use of the pre-v1.0 escape clause; PATCH because zero new public C API symbols, no new opcodes, wire unchanged); v0.11.3-memory-debug bumped PATCH to 0/20/3 (fourth and final tag of the v0.11.x tooling arc — on-target memory debugging behind `URBI_MEM_DEBUG`: owner-tag sidecar, redzone/poison/quarantine, heap-lock + handle/pin leak detection, `Debug.memCheck()`, GDB `urbi-heap` full walk + `urbi-allocs`/`urbi-leaks`; all internal functions, 27th use of the pre-v1.0 escape clause; PATCH because zero new public C API symbols, no new opcodes, wire unchanged); v0.11.4-cat-f bumped PATCH to 0/20/4 (catchable structured exceptions — VM-internal raise helpers + 6 slot-access TypeError sites now raise catchable typed Exception instances, cached subclass protos resolved-by-name + GC-rooted, strand catch_value GC-rooted, 4 new Exception subclasses, uncaught typed throws print `!!! <message>`; two internal exported symbols `urbi_raise_typed`/`urbi_exception_subclass_protos_resolve` already in Tier 4 — internal-leak not public surface; 28th use of the pre-v1.0 escape clause; PATCH because zero new public C API symbols, no new opcodes, wire unchanged); v0.12.0-ros-foundation bumped MINOR to 0/21/0 (optional ROS2 bridge — 3 new public C API symbols in `include/urbi/ros.h` gated by `URBI_ENABLE_ROS2`: `urbi_ros_register`, `urbi_ros_register_globals`, `urbi_ros_pump`; per v0.11.0-trace-spine precedent, new public symbols behind a compile gate count as MINOR; 29th use of the pre-v1.0 escape clause; wire unchanged); v0.12.1-ros-dds bumped PATCH to 0/21/1 (real rcl/rclc/Fast-DDS transport behind `URBI_ROS_BACKEND=rcl` — backend, codegen, List builder, and seam revision all internal; 30th use of the pre-v1.0 escape clause; zero new public C API symbols; wire unchanged); v0.12.2-urobotics bumped MINOR to 0/22/0 (optional Standard Robotics API facet overlay `URBI_ENABLE_UROBOTICS` — pure urbiscript baked into a separate blob; 2 new gated public C API symbols `urbi_urobotics_register`/`urbi_urobotics_run` in `include/urbi/urobotics.h`; per the v0.11.0-trace-spine / v0.12.0-ros-foundation precedent, gated public symbols count as MINOR; 31st use of the pre-v1.0 escape clause; wire unchanged); v0.12.3-ros-demo-and-contract bumped PATCH to 0/22/1 (zero new public C API symbols; binding pure-overlay, mock hooks gated test-only); v0.12.5-m10-prep ships 0/23/0 (escape #33, B6a: urbi_set_clock_fn rename + -fvisibility=hidden + bidirectional manifest gate); `docs/api-stability.md` documents post-freeze policy; v0.13.0-test-honesty bumped PATCH to 0/23/1 (refactor-3 W6 infra-honesty wave — chk-runner exit/timeout/VACUOUS/SKIP semantics, gate-script hardening, Makefile race/stale-object fixes, CI parity; zero new public C API symbols, no new opcodes, wire unchanged, not a pre-v1.0 escape); v0.13.1-unwind-and-frontend bumped PATCH to 0/23/2 (refactor-3 W1 unwind + frontend correctness wave — cross-frame unwind, atomic cleanup bodies, scope-crossing exits, emitter back-edge/switch/not/capture fixes; new internal cross-TU symbols carry the urbi_ prefix and are Tier-4 manifested; zero new public C API symbols, no new opcodes, wire unchanged, not a pre-v1.0 escape); v0.13.2-gc-soundness bumped PATCH to 0/23/3 (refactor-3 W2 GC-soundness wave — container element roots + insertion barrier, ATOMIC_FINISH full root-provider re-scan, strand/watcher/event rooting, C-stack root chain, IC re-key, upvalue-close barrier, URBI_GC_STRESS collect-before-alloc mode + rooting-matrix harness + test-gc-stress gate; one Tier-4 internal accessor `urbi_intern_bytes` + one Tier-4 root-provider symbol manifested; zero new public C API symbols, no new opcodes, wire unchanged, not a pre-v1.0 escape); v0.13.3-scheduler-liveness bumped PATCH to 0/23/4 (refactor-3 W3 scheduler-liveness wave — single-writer runnable count, QUIESCENT excludes armed/suspended/waiting work, centralized unpark, independent block/freeze gates + tag-stop resumes SUSPENDED; one vm_reactive_drain() + idle-pump + post-loop drain; idle whenever bounded to rising edge; per-slice safepoint budget re-arm; step-budget-exhausted strand re-enqueued; scratch strands budgeted + queue-excluded; at-sync `;` bodies run to completion atomically; every() float=seconds + fixed/overrun cadence + finite guards; watcher unregister UAF closed; emit delivers to pre-emission subscribers only; verifier bounds/adjacency checks for JOIN/SELF/FORK_JOIN + OP_TAG_STOP round-trips; 9 liveness-helper symbols carry `urbi_` prefix + Tier-4 manifested; zero new public C API symbols, no new opcodes, wire unchanged, not a pre-v1.0 escape); v0.13.4-error-surfacing bumped PATCH to 0/23/5 (post-v0.13.3 error-surfacing hardening (restructured arc, tag 5) — batch/embedding entry points return URBI_ERR_UNCAUGHT_THROW on uncaught script throws; runtime arithmetic/comparison/call type errors are catchable typed throws; comparison overloads dispatch on either operand; try/catch + tag-scope yield body value; prefix -/! bind after postfix chain; loader depth cap + OP_CALL window verify + fuzz harness; every() positive-period guard + tag.stop cancels owned periodics + sleep(0) no longer aborts batch; watcher drain mid-pass cascade fix; REPL overflow envelope + OOM teardown; CLI -e/-f on loader strand; one new URBI_ERR_UNCAUGHT_THROW = -18 enumerator filling reserved hole, pre-freeze owner-sanctioned; no new public C functions, no new opcodes, wire unchanged, not a pre-v1.0 escape); v0.13.5-conformance-and-stdlib bumped PATCH to 0/23/6 (legacy-conformance pass — truthiness alignment, statement-operand fold, unbraced bodies, switch default, default parameters, universal asString, String %, compat aliases, List.sort(comparator), typed exceptions from div-by-zero, RegExp budget cap, emit diagnostics with positions, message polish, tag-watcher persistence, REPL line-cap; zero new public C API symbols, no new opcodes, wire unchanged, not a pre-v1.0 escape); v0.13.6-consistency bumped PATCH to 0/23/7 (internal-consistency pass — cross-TU internals namespaced under the `urbi_` prefix with two internal-leak allowlist rows removed, duplicated internals consolidated (shared native-method registration, unified value constructors, parser/emitter helpers, opcode name tables, intrusive-list idiom, REPL transport; ≈−700 LOC), dead code removed (UStrand 3920 → 3912), fork-operand register-allocation fix for `&`/`,` statement operands, Boolean/Nil asString, unified positioned division/modulo error prefixes, comment/doc truthfulness program + source-comment lint gate, internals-docs accuracy sweep, GC rooting-matrix additions, embedded footprint preset + per-port stack-cap knob + firmware size gate, bytecode-verifier and emitter control-flow-arm decompositions; the XC-04 public shims are untouched — deprecation banner comment only; zero new public C API symbols, no new opcodes, wire unchanged, not a pre-v1.0 escape) |
...
5. ✅ **No `docs/urbi-embedded-design-risks.md` entry tagged "Handle before v1.0" remains a blocker** — closed by the M10 / B4 closure triage (2026-06-01). The three real scheduler/unwind blockers were FIXED before the freeze: `v0.11.4-D` (finally on the normal path), `v0.10.15-B` (tag-stop absorption / resume-after-scope), `v0.11.4-A` (lone-sleeper wake). The remaining entries that still carried a "Before v1.0-rc opens" deadline are all NON-blocking and re-dispositioned to v1.x, each with a workaround or being test-harness/tooling-only: `v0.10.7-G`(d) `module_load_isolation` (needs the import-table surface — feature gap, fixture stays blocked), `v0.10.7-I` (TSAN image baking — tooling), `v0.10.9-C` (valued-block script-side resume-value delivery — the C API works; only the `var x = t.block(default)` sugar is deferred), `v0.10.10-A` (OGET native-closure dispatch — worked around with call-style methods), `v0.10.10-D` (props_table propagation — worked around by ordering), `v0.11.3-C` (root-provider imbalance detection — diagnostics-only). Accepted known-issues are listed in the v1.0.0 release notes. See the design-risks register for per-entry rationale.
