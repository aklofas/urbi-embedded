...
| ABI freeze pin | **done (W2)** | `make test-abi-freeze` — "ABI freeze pin in sync: 0/18/0" | **passing-evidence** — `_Static_assert` in `include/urbi/version.h`; freeze pinned at v0.10.6-stabilization; `docs/api-stability.md` documents post-freeze policy |
