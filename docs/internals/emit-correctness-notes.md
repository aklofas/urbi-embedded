...
AST_AT_EVENT emit register-allocation desync where `urbi_emit_function_literal`
...
because their condition was wrapped in `urbi_emit_function_literal`
symmetrically.
...
- **AST_SLOT_CHANGE** — fixed at `v0.5.2-scratch-frame-followup` (commit
  `5bd98af`).
- **AST_WATCHER (AT / WHENEVER)** — symmetric `urbi_emit_function_literal` wrap:
...
`urbi_emit_function_literal` to construct an event-bound body, this list must
