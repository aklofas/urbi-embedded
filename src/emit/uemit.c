...
/* ---------------------------------------------------------------------------
 * try/catch/finally emit helpers (EMIT-033).
 * Factored out of the AST_TRY arm in emit_expr to collapse the 3 near-
 * duplicate paths (catch+finally / catch-only / finally-only) into a single
 * emit_try_frame function.  Bytecode output is identical to the original
 * inline code.
 * -------------------------------------------------------------------------- */
 ...
 * rd = result register (pre-allocated by the AST_TRY arm).
 *
...
static uint8_t emit_expr(UEmitter *e, UAstNode *n) {
    ...
    case AST_TRY: {
         ...
         * Three paths (catch+finally / catch-only / finally-only) are
         * collapsed into emit_try_frame (EMIT-033). */
        if (e->current_fs == NULL) {
            e->error = EMIT_UNSUPPORTED_AST;
            return 0u;
        }
