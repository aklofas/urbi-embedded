...
/* UVmTagScopeResult — return codes from urbi_vm_push_tag_scope / urbi_vm_pop_tag_scope.
 *
...
/* urbi_vm_push_tag_scope: execute the OP_PUSH_TAG body for the instruction at s->pc.
 *
...
UVmTagScopeResult urbi_vm_push_tag_scope(UVM *vm, UStrand *s);

/* urbi_vm_pop_tag_scope: execute the OP_POP_TAG body for the instruction at s->pc.
 *
...
UVmTagScopeResult urbi_vm_pop_tag_scope(UVM *vm, UStrand *s);

/* urbi_vm_tag_scope_teardown: run the OP_POP_TAG teardown for the top
 ...
 * urbi_vm_pop_tag_scope (normal exit) and the unwind walker's tag.stop() absorption
 * (v0.10.15-B) so the two paths cannot drift. */
struct UCleanupEntry;
void urbi_vm_tag_scope_teardown(UStrand *s, struct UCleanupEntry *top);
