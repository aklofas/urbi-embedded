...
/* urbi_event_emit_async: fan out payload to all subscribers asynchronously.
 *   AT_EVENT / AT_EVENT_SYNC watcher bodies → spawn strand via
 *     urbi_watcher_do_spawn_body_coroutine (fire_context NULL at baseline).
 *   Waiters (USTRAND_WAIT_EVENT) → deposit payload + wake.
 * FIFO registration order (D-event-1 determinism).
 * Not ISR-safe. */
void urbi_event_emit_async(struct UVM *vm, struct UEvent *e, UValue payload);

/* urbi_event_emit_sync: as async but AT_EVENT_SYNC bodies run inline on the
 ...
 * Not ISR-safe. */
void urbi_event_emit_sync(struct UVM *vm, struct UEvent *e, UValue payload);

/* urbi_event_waituntil: block the calling strand until e is emitted.
 *   Tail-appends caller to e->waiters_head; transitions to USTRAND_WAIT_EVENT.
 *   The opcode caller MUST goto exit_strand after this returns.
 ...
 * Not ISR-safe. */
UValue urbi_event_waituntil(struct UVM *vm, struct UEvent *e);
