...
**Status:** v0.11.4-cat-f — Catchable structured exceptions. VM-internal error sites that previously fatal-HALTed the strand or printed diagnostic strings to stderr (the native-method raise helpers + the 6 slot-access TypeError sites) now raise catchable typed `Exception` instances; cached subclass protos resolved-by-name after the stdlib bake + GC-rooted; the strand `catch_value` added to the GC root walker; four new `Exception` subclasses (`RuntimeError`, `SchedulingError`, `SyntaxError`, `OutOfMemoryError`); uncaught typed throws print a `!!! <message>` diagnostic at top level. Plus two test-only chk host-driver directives. ABI 0/20/4 (PATCH bump from 0/20/3; 28th use of pre-v1.0 escape clause — two internal exported symbols already manifested in Tier 4, internal-leak not public surface, zero new public C API symbols). Wire v1.9 / 0x19 unchanged.

Previously: **v0.11.3-memory-debug** — Fourth and final tag of the v0.11.x tooling arc. On-target memory debugging behind the new `URBI_MEM_DEBUG` compile gate (off by default, zero bytes when off): allocation owner-tagging on the existing GC sidecar, trailing redzones, poison-on-free + freed-cell quarantine (use-after-free detection where ASan/Valgrind can't run), heap-lock violation recording, and host-handle/pin leak reporting. Surfaced GDB-first (`tools/gdb/urbi.py` gains a full `urbi-heap` live-cell walk plus `urbi-allocs` and `urbi-leaks`) plus one `Debug.memCheck()` script trigger. ABI 0/20/3.

Next milestone: **v0.12.x ROS2 (M9)** — micro-ROS / ROS2 integration; subsystem prefix `ros:` comes online. Then **v1.0-rc**. Tagged `v0.11.4-cat-f`.
