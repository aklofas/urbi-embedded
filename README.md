...
**Status:** v0.10.1-invariants — Wave 2 of the v0.10.x architectural refactor arc: documented invariants become machine-enforced. New `URBI_REQUIRE(cond, msg)` macro in `<urbi/require.h>` fires in all build modes (host / release / freestanding). Link-time guards on `URBI_FLOAT_TYPE`, `URBI_REPL_COOPERATIVE_ONLY`, and `URBI_BYTECODE_ONLY` turn silent embedder/library build-flag mismatches into named undefined-symbol link errors. UProto refcount goes through a typed-handle API (saturation now logs, underflow now `URBI_REQUIRE`-fails); OP_CLOSURE dispatch asserts promoted to release-firing `TYPE_ERROR`; bytecode loader runs a `verify_chunk_bounds` deserialize-time pass that rejects malformed OP_CLOSURE / OP_JMP / OP_LOAD_REALM_GLOBAL / OP_CALL and verifies `ic_index` DFS pre-order; `ustrand_destroy` now unregisters event waiters and wakes joiners (matching `exit_strand`); `sched_post_dispatch` helper extracted from `urbi_step` for alternative drivers; combined `urbi_gc_slot_store` helper eliminates the barrier-then-store footgun. ABI 0/14/0 and wire v1.8 (0x18) unchanged (the new `<urbi/require.h>` header is additive only). Builds on v0.10.0-truthfulness (Wave 1: docs/manifests/CI alignment), v0.9.4-pico-example (Raspberry Pi Pico hardware port — RP2040 / Cortex-M0+ / USB CDC REPL), M8 networked REPL service, M6 stdlib, M5 reactive runtime, M4 prototype object model + GC, and M3 cooperative scheduler. Strict-tooling all-categories hard-gated. Tagged `v0.10.1-invariants`.
