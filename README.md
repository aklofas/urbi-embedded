...
**Status:** v0.11.1-perf-counters — Second tag of the v0.11.x tooling arc. Adds VM-domain performance counters behind the `URBI_PERF_COUNTERS` compile gate (default off ⇒ increments are `(void)0`, the counter struct is absent from `UVM`): opcodes retired, calls/returns, slot get/set, IC hit/miss, native calls, scheduler context-switches/yields/blocks, and watcher-install/fire/event-emit counts. Always-on GC cycle/slice counts plus cycle timing feed `Debug.gc()`, and the deliberately-stubbed `Debug.profile()` seam is now filled (counters + `epoch`), with a new `Debug.profileReset()`. All counters are excluded from the determinism checksum (proven by a 100-run `test-determinism-perf` gate). ABI 0/20/1 (PATCH bump from 0/20/0; 25th use of pre-v1.0 escape clause — counters are internal, surfaced only via `Debug.*`; zero new public C API symbols). Wire v1.9 / 0x19 unchanged.

Previously: **v0.11.0-trace-spine** — First tag of the v0.11.x tooling arc. Kernel-style runtime trace subsystem (new optional header `<urbi/trace.h>`): `URBI_TRACE` compile gate, per-channel runtime masks, a fixed-size per-VM binary ring, tracepoints across scheduler / GC / reactive runtime / REPL, bring-up primitive macros, and a `Debug.trace("…")` marker. ABI 0/20/0.
