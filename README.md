...
**Status:** v0.10.2-reactive — Wave 3 of the v0.10.x architectural refactor arc: reactive primitives that were documented but broken-by-construction on cooperative embedded builds become functional end-to-end. `whenever (named_event) { body }` parses and dispatches via new `OP_WHENEVER_EVENT_INSTALL` (opcode 48) as a perpetual event subscriber distinct from one-shot `at (e?)`. `OP_CLOSURE` inside `every`/`at`/`whenever` body strands resolves nested closures correctly (body-strand entry frame now binds the body closure's proto as `root_proto`). `AT_EVENT` watchers fully unlink from `event->at_watchers_head` synchronously on tag-stop cascade (closes a UAF window between drain-push and next-safepoint drain-pop); defence-in-depth `URBI_WATCHER_PENDING_UNREGISTER` checks in `c_event_emit_async`/`_sync`. The deferred slot-change ring gains a GC root provider (`urbi_deferred_slot_changes_walk_roots`), removing the cooperative-only weak-ref hazard. `Tag.new()` / `.stop()` / `.freeze()` / `.unfreeze()` / `.block()` / `.unblock()` native methods on `vm->tag_proto`; `OP_TAG_STOP` (opcode 30) replaces its M3 reserved-stub with a real `urbi_tag_stop` call; new `UVAL_TAG = 12` UValKind (runtime-only — not serialized, no wire-format constant-pool change). Bare-prefix tag-label form `tag: stmt` parses without requiring braces. `sleep(duration)` script-level built-in via `USTRAND_REASON_SLEEP`; TAG_STOP wakes sleeping strands. Two pre-existing latent bugs surfaced by `sleep()`: post-native-call `USTRAND_IS_WAITING` check in `OP_CALL` dispatch (was continuing past parked strand) + `uchunk_loader_drive` `out_slot=NULL` on park (was writing past caller's stack frame). ABI 0/14/0 → 0/14/1 (additive — `UVAL_TAG` enum + new opcode at slot 48). Wire format bumps from wire v1.8 / 0x18 to wire v1.9 / 0x19 (opcode space extension; no on-disk layout change otherwise). Builds on v0.10.1-invariants (Wave 2: machine-enforced invariants), v0.10.0-truthfulness (Wave 1), v0.9.4-pico-example (Raspberry Pi Pico hardware port), M8 networked REPL service, M6 stdlib, M5 reactive runtime, M4 prototype object model + GC, and M3 cooperative scheduler. Strict-tooling all-categories hard-gated. Tagged `v0.10.2-reactive`.
