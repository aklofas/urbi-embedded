...
**Status:** v0.10.3-api-opacity — Wave 4 of the v0.10.x architectural refactor arc: API freeze preconditions. Public C API becomes opaque, error model unified, surface classified into stable / advanced / experimental tiers, `vm`-first-arg convention swept across the strand and control-transfer family, embedding guide rewritten against the new shape. New `urbi_vm_create(alloc_fn, ud)` / `urbi_vm_free` opaque allocation API (W1) means embedders no longer need `#include "vm/uvm.h"` — `urbi_vm_init`/`_destroy` retained as `URBI_ADVANCED` for static-allocation embedders with tight memory budgets. `<urbi/gc.h>` and `<urbi/sched.h>` stop including `src/`-prefixed headers (W2); new CI gate `test-external-embed-iinclude` compiles a minimal external program with `-Iinclude` alone. Error model unified: all public API returns `int`; `URBI_OK` (0) on success, negative `URBI_ERR_*` on failure (+ per-VM error ring), positive `UCallbackSignal` (`URBI_CB_OK`/`_UNREGISTER`/`_THROW`) for host-callback returns only (W3). `UVMError` + `UExecStatus` retired from the public surface (kept as typedef int + `#define` shims for one release). 5 callback setters gain trailing `void *ud`: `urbi_set_diag_fn`, `urbi_set_time_us`, `urbi_set_watcher_body_done_fn`, `urbi_set_isr_check_fn`, `urbi_register_event_drain`. 13 `urbi_value_is_*` predicates + 9 `urbi_aux_value_to_*` checked accessors (W4); new `URBI_ERR_TYPE` (-26). 17 functions take `vm` as new first arg (strand family + control-transfer + tag_info + chunk_from_bytes/_free + realm compile_budget + sched priority); 3 void-returning functions change to `int` per F8 safety contract (`urbi_strand_destroy`, `urbi_throw`, `urbi_return_val`); new `urbi_strand_state(vm, s)` + `UStrandState` enum + `UStrandUnwind` public mirror + `URBI_ERR_INVALID_STATE` (-27); `urbi_chunk_from_bytes` now routes alloc through `vm->alloc_fn` (W5). `URBI_EXPERIMENTAL`/`URBI_ADVANCED`/`URBI_DEPRECATED` attribute macros decorate 12 functions; `docs/api-surface-tiers.md` manifest + `test-api-manifest` CI gate (W7). `docs/embedding-guide.md` rewritten against the post-Wave-4 surface + `test-embedding-guide` CFLAGS tightened to `-Iinclude` only (W6). ABI 0/15/0 (was 0/14/1 — 12th + 13th use of pre-v1.0 escape clause). Wire format unchanged at wire v1.9 / 0x19 (C-API only wave). Strict-tooling all-categories hard-gated. Tagged `v0.10.3-api-opacity`.
