...
**Status:** v0.12.0-ros-foundation — Optional ROS2 bridge foundation (`URBI_ENABLE_ROS2`). The `ros` urbiscript namespace provides `ros.init`, `ros.msg`, `ros.publisher`/`.publish`, `ros.subscribe` (returns an `Event`; use with `at(sub?(var m))`), `ros.client`/`.call`, and `ros.service`. Build-time message codegen (`tools/urbi-rosgen.py`) covers a scalar + nested-scalar message subset; reactive pump wires incoming messages onto watchers. v0.12.0 ships only the mock transport (host-only, `make test-ros2`); real rclc/DDS lands in v0.12.1. ABI 0/21/0 (MINOR bump from 0/20/4; 29th use of pre-v1.0 escape clause — 3 new public C API symbols in `include/urbi/ros.h`, gated by `URBI_ENABLE_ROS2`). Wire v1.9 / 0x19 unchanged.

Previously: **v0.11.4-cat-f** — Catchable structured exceptions. VM-internal error sites that previously fatal-HALTed the strand or printed diagnostic strings to stderr (the native-method raise helpers + the 6 slot-access TypeError sites) now raise catchable typed `Exception` instances; cached subclass protos resolved-by-name after the stdlib bake + GC-rooted; the strand `catch_value` added to the GC root walker; four new `Exception` subclasses (`RuntimeError`, `SchedulingError`, `SyntaxError`, `OutOfMemoryError`); uncaught typed throws print a `!!! <message>` diagnostic at top level. ABI 0/20/4.

Next milestone: **v0.12.1-ros-rclc** — real rclc/DDS transport. Then **v1.0-rc**.
