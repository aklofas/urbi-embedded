...
**Status:** v0.11.0-trace-spine — First tag of the v0.11.x tooling arc. Adds a kernel-style runtime trace subsystem (new optional header `<urbi/trace.h>`): compile-time-disabled to zero bytes via `URBI_TRACE`, runtime-maskable per channel when compiled in, a fixed-size per-VM binary ring, named tracepoints across the scheduler, GC, reactive runtime (watcher/event/tag), and REPL lifecycle, the bring-up primitive macros (milestone / periodic / first-N / threshold), and a `Debug.trace("…")` script marker. Default builds stay byte-identical (zero `UVM` delta, no ring/emit symbols in the archive). ABI 0/20/0 (MINOR bump from 0/19/6; 24th use of pre-v1.0 escape clause — new public EXPERIMENTAL trace control/drain API). Wire v1.9 / 0x19 unchanged.
