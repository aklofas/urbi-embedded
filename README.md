...
**Status:** v0.11.2-host-tooling — Third tag of the v0.11.x tooling arc. Host-side tooling for the v0.11.0 trace ring + v0.11.1 counters: a Python Perfetto/Chrome-Trace decoder (`tools/urbi-trace-decode.py`) for the binary `URBT` dump, GDB pretty-printers + heap/strand/handle/trace walkers (`tools/gdb/urbi.py`, including a one-shot `urbi-dump`), a `--trace`/`--trace-out` capture path on the `urbi` CLI (built via `make urbi-trace`), and a `--dump-on-fatal` best-effort host dump. No on-target changes — the CLI flags use only already-exported public symbols. ABI 0/20/2 (PATCH bump from 0/20/1; 26th use of pre-v1.0 escape clause — zero new public C API symbols). Wire v1.9 / 0x19 unchanged.

Previously: **v0.11.1-perf-counters** — Second tag of the v0.11.x tooling arc. VM-domain performance counters behind the `URBI_PERF_COUNTERS` compile gate, always-on GC cycle/slice/timing feeding `Debug.gc()`, the `Debug.profile()` seam filled (counters + `epoch`) plus `Debug.profileReset()`; all counters excluded from the determinism checksum. ABI 0/20/1.
