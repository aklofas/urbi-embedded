...
**Status:** v0.10.0-truthfulness — Wave 1 of the v0.10.x architectural refactor arc: documentation, manifests, and CI gates aligned with code state (zero runtime C semantics modified; ABI 0/14/0 and wire v1.8 unchanged). Capabilities unchanged from v0.9.4-pico-example: Raspberry Pi Pico hardware port (RP2040 / Cortex-M0+ / USB CDC REPL), cooperative `urbi_repl_serve_step` data plane, and `every (period) body` language feature. Builds on M8 networked REPL service (TCP / Unix socket / UART / pty, bearer-token auth, 9 introspection ops, `Debug` urbiscript namespace), M6 stdlib (atoms, containers, runtime primitives), M5 reactive runtime (`at` / `whenever` / `waituntil` / `every` / tag-scope `enter` / `leave`), M4 prototype object model + GC, M3 cooperative scheduler + incremental tri-color GC (sub-3 µs pause), parallel separators (`,` / `&`), exception unwind, realm/namespace chunk lifecycle, ISR-safe event ring, and determinism infrastructure. Bytecode wire format v1.8 (0x18), ABI 0/14/0. Strict-tooling all-categories hard-gated. Tagged `v0.10.0-truthfulness`.
