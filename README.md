...
**Status:** v0.11.3-memory-debug — Fourth and final tag of the v0.11.x tooling arc. On-target memory debugging behind the new `URBI_MEM_DEBUG` compile gate (off by default, zero bytes when off): allocation owner-tagging on the existing GC sidecar, trailing redzones, poison-on-free + freed-cell quarantine (use-after-free detection where ASan/Valgrind can't run), heap-lock violation recording, and host-handle/pin leak reporting. Surfaced GDB-first (`tools/gdb/urbi.py` gains a full `urbi-heap` live-cell walk plus `urbi-allocs` and `urbi-leaks`) plus one `Debug.memCheck()` script trigger. ABI 0/20/3 (PATCH bump from 0/20/2; 27th use of pre-v1.0 escape clause — zero new public C API symbols). Wire v1.9 / 0x19 unchanged.

Previously: **v0.11.2-host-tooling** — Third tag of the v0.11.x tooling arc. Host-side tooling for the v0.11.0 trace ring + v0.11.1 counters: a Python Perfetto/Chrome-Trace decoder (`tools/urbi-trace-decode.py`) for the binary `URBT` dump, GDB pretty-printers + heap/strand/handle/trace walkers (`tools/gdb/urbi.py`, including a one-shot `urbi-dump`), a `--trace`/`--trace-out` capture path on the `urbi` CLI (built via `make urbi-trace`), and a `--dump-on-fatal` best-effort host dump. No on-target changes. ABI 0/20/2.

Next milestone: **v0.11.3-memory-debug** — final tag of the v0.11.x tooling arc (`URBI_MEM_DEBUG` allocation owner-tag sidecar + poison/redzone/quarantine; GDB reads them). Then **v0.12.x ROS2 (M9)** — micro-ROS / ROS2 integration; subsystem prefix `ros:` comes online. Then **v1.0-rc**. Tagged `v0.11.2-host-tooling`.
