...
**Status:** v0.12.3-ros-demo-and-contract — facet↔ROS2 binding contract: a pure-urbiscript `Robotics.bindInput` / `Robotics.bindOutput` layer mapping Standard Robotics API facet slots to ROS2 message fields (gated on both `URBI_ENABLE_UROBOTICS` and `URBI_ENABLE_ROS2`; zero core-VM change), plus an assembled-robot demo and `sensor_msgs/Range`. New mock-only test hooks (`ros.__injectMsg` / `ros.__lastPublished`) drive the combined `test-ros-urobotics` host gate; a live-DDS driver covers the rcl backend. ABI 0/22/1 (PATCH bump from 0/22/0; 32nd use of pre-v1.0 escape clause — NO new public C API symbols; the binding is pure overlay, the mock hooks are gated test-only natives inside the ROS2 component). Wire v1.9 / 0x19 unchanged.
